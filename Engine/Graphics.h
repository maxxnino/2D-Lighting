...
public:
	...
	void DrawRect(const VecI& pos,int size, Color c);
	void DrawActor(const VecI& pos, int size, Color c);
	void DrawCircle(const VecI& pos, int rad, Color c);
	void DrawLineClip(float x0, float y0, float x1, float y1, Color c);
	void DrawLineClip(const VecF& p1, const VecF& p2, Color c)
	{
		DrawLineClip(p1.x, p1.y, p2.x, p2.y, c);
	}
	...
	void DrawTriangle(const VecF& v0, const VecF& v1, const VecF& v2, Color c);
	template<typename E>
	...
	~Graphics();
private:
	void DrawFlatTopTriangle(const VecF& v0, const VecF& v1, const VecF& v2, Color c);
	void DrawFlatBottomTriangle(const VecF& v0, const VecF& v1, const VecF& v2, Color c);
private:
...
extern template
void Graphics::DrawSprite<SpriteEffect::InShadow>(int x, int y, RectI srcRect, const RectI& clip, const Surface& s, SpriteEffect::InShadow effect);
#endif
...
namespace SpriteEffect
{
	...
	public:
		...
		{
			const Color dest = gfx.GetPixel(xDest, yDest);
			float ababcdefs = 1.0f - intensity;
			const Color blend = {
				unsigned char(ababcdefs * src.GetR() + intensity * dest.GetR()),
				unsigned char(ababcdefs * src.GetG() + intensity * dest.GetG()),
				unsigned char(ababcdefs * src.GetB() + intensity * dest.GetB()) };
			gfx.PutPixel(xDest, yDest, blend);
		}
	private:
		float intensity;

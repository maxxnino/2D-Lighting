...
void Graphics::DrawRect(const VecI& pos, int size, Color c)
{
...
void Graphics::DrawActor(const VecI& pos, int size, Color c)
{
...
void Graphics::DrawCircle(const VecI& pos, int rad, Color c)
{
	...
	for (int y = pos.y - rad; y <= pos.y + 2 * rad; y++)
	{
		for (int x = pos.x - rad; x <= pos.x + 2 * rad; x++)
		{
			...
			{
				Color bgColor = GetPixel(x, y);
				int distanceSq = (x - pos.x) * (x - pos.x) + (y - pos.y) * (y - pos.y);
				if (distanceSq < radSq)
				{
					const float strengBg = (float)distanceSq / (float)radSq;
					const float streng = 1.0f - strengBg;
					Color newcolor = c;
					if (streng <= 0.999)
					{
						newcolor.SetR((unsigned char)(streng * c.GetR() + strengBg * bgColor.GetR()));
						newcolor.SetB((unsigned char)(streng * c.GetB() + strengBg * bgColor.GetB()));
						newcolor.SetG((unsigned char)(streng * c.GetG() + strengBg * bgColor.GetG()));
					}
...
void Graphics::DrawTriangle(const VecF& v0, const VecF& v1, const VecF& v2, Color c)
{
	...
	else // general triangle
	{
		...
		{
			DrawFlatBottomTriangle((VecF)*pv0, (VecF)*pv1, vi, c);
			DrawFlatTopTriangle((VecF)*pv1, vi, (VecF)*pv2, c);
		}
		else // major left
		{
			DrawFlatBottomTriangle((VecF)*pv0, vi, (VecF)*pv1, c);
			DrawFlatTopTriangle(vi, (VecF)*pv1, (VecF)*pv2, c);
		}
	}
}

void Graphics::DrawFlatTopTriangle(const VecF& v0, const VecF& v1, const VecF& v2, Color c)
{
...
void Graphics::DrawFlatBottomTriangle(const VecF& v0, const VecF& v1, const VecF& v2, Color c)
{
	// calulcate slopes in screen space
